  ...
  return (
    <>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
        <div className="flex bg-gray-100 border-gray-300 dark:border-gray-600 border dark:bg-gray-700 p-1 border-radius-3xl">
          {(["all", "month", "custom"] as FilterType[]).map((type) => (
            <button
              key={type}
              onClick={() => handleTypeChange(type)}
              className={`px-2 py-1 lg:py-2 lg:px-4 border-radius-3xl text-sm font-bold capitalize transition-colors cursor-pointer relative ${
                filter.type === type
